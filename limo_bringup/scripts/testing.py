...
def send_goal_with_recovery(x, y):
    ...
    def perform_recovery_with_strategy(attempt):
        ...
        if case == 1:
            ...
            rospy.logwarn("Recovery Step 4 (improved): Clear costmap → resend goal → reverse only if needed")

            try:
                rospy.wait_for_service('/move_base/clear_costmaps', timeout=2.0)
                clear_srv = rospy.ServiceProxy('/move_base/clear_costmaps', Empty)
                clear_srv()
                rospy.loginfo("Costmap cleared before resending goal.")
                rospy.sleep(2.0)
            except (rospy.ServiceException, rospy.ROSException) as e:
                rospy.logerr("Costmap clear failed: %s", str(e))

            rospy.loginfo("Resending original goal...")
            send_goal()

            short_wait = rospy.Duration(5.0)
            wait_success = client.wait_for_result(short_wait)
            state = client.get_state()

            if wait_success and state == actionlib.GoalStatus.SUCCEEDED:
                rospy.loginfo("Goal SUCCEEDED shortly after costmap clear — no reverse needed.")
                return
            else:
                rospy.logwarn("Goal still not succeeded. Proceeding with reverse.")
